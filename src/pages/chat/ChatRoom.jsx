...
export const ChatRoom = () => {
  ...
  // --- RENDER ---
  if (!currentUser || !chat) return <div className="container" style={{textAlign:'center', marginTop:'50px'}}>Cargando...</div>;
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', height:'100vh', background:'var(--bg-body)'}}>
      <header className="header">
        <button onClick={() => navigate("/chats")} style={{background:'none', border:'none', fontSize:'1.2rem', color:'var(--text-main)', cursor:'pointer'}}>← Volver</button>
        <div className="logo" onClick={() => navigate('/home')} style={{cursor:'pointer', fontSize:'1.2rem'}}>POLIMARKET</div>
      ...
      <div style={{padding:'15px', background:'var(--bg-card)', borderTop:'1px solid var(--border-color)'}}>
        ...
        {isBuyer && chat.payment_confirmed && (
            <div style={{display:'flex', gap:'10px', marginBottom:'10px', animation: 'fadeIn 0.5s'}}>
                ...
                <button className="login-btn" style={{background:'var(--accent)', color:'black', flex:1}} onClick={() => setShowDeliveryModal(true)}>
                    🛵 Pedir Delivery
                </button>
            </div>
        )}
